...
public interface Generator<T> {
     ...
     *
     * <p>When this generator is a {@link io.hegel.generators.BasicGenerator},
    ...
    default Generator<T> filter(Predicate<T> predicate) {
        ...
        return tc -> {
            for (int i = 0; i < 3; i++) {
                tc.getDataSource().startSpan(io.hegel.generators.SpanLabels.FILTER);
    ...
    default <U> Generator<U> flatMap(Function<T, Generator<U>> f) {
        Generator<T> self = this;
        return tc -> {
            tc.getDataSource().startSpan(io.hegel.generators.SpanLabels.FLAT_MAP);
